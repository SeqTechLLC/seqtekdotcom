...
>
> **Resolved (2026-09-01, ROADMAP SVC-2).** `services` is a routed metadata collection again, in the `partners` shape: a `layout` block body, `/services/[slug]`, sitemap and static params derived from published slugs, and no hardcoded lists anywhere. The four `service-*` Pages and the `[offering]` route are deleted. It goes one step further than `partners` — `servicePillars` was absorbed into the same collection under a `tier` of `leaf | group | axis`, so the group and axis pages are rows rather than a second collection or another special-cased `Page`. A new service is now a publish, which is what this ADR asked for.
