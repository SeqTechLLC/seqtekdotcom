...
> Promoted from `specs/001-google-oauth-sso/contracts/` when the spec directories were retired.
> `FR-*`, `US*` and `T0*` identifiers below refer to that spec; what shipped is in `docs/PROJECT_HISTORY.md`.
