...
| `case-studies.json["…/workshops"]`                                                           | `pages` doc with `slug: touchstone-workshops`                                                                                                                                                                                                                                                                                                                          | Layout: `hero`, `stats-bar` (70% / $2.3T / #1), `content` (The SEQTEK Approach), `workshop-progression` (placeholder — workshops collection seeded separately), `content` (Build Health From the Inside Out), `cta-section`. **Superseded (2026-06-30):** the `touchstone-workshops` slug is retired (now a `/workshops` 301-redirect source per fa42926), and the cited `workshop-progression` block does not exist — the real block is `workshop-list`                                                                                                                                          |
...
| `case-studies.json["…/our-services"]`                                                        | **Not** a `pages` doc — populates `servicePillars` + `services` references only. See §12 — content is too duplicated/copy-pasted to migrate verbatim. **Superseded (2026-06-30):** the `servicePillars` + `services` pillar IA is retired (PRs #79–#83); `/services` and `/services/[offering]` now render Pages by slug, and those collections are vestigial/unrouted |
...
const AUDIT = process.env.AUDIT_DIR ?? resolve(process.cwd(), 'audit')
...
- **Workshop branding** — Touchstone branding work pending (CONTENT-REQUIREMENTS.md Tier 2). The three workshops (`Five Dysfunctions`, `Re-Alignment`, `Case Study`) seed as stubs with deliverables arrays only. _[Note 2026-06-30: this lags the `/workshops` IA correction (fa42926) — Touchstone is now one workshop of three at `/workshops`, and "Case Study" is no longer in the workshop set.]_
