...
**Status:** Phase 3 render foundation (spec 004) shipped (PR #21) and closed; marquee _content_ deferred to the content track (content-lead-gated). Spec 008 GTM code/doc track shipped (PR #31, P5-2) — external GTM-UI/staging config tail remains. Spec 009 media-via-CloudFront fix shipped and verified on staging (PRs #43/#44, P5-6) — media is edge-served end-to-end; no in-repo residual. Post-spec-004 template render/IA/design polish (PRs #47–#60) archived as **P3-3**/**P5-7**; the Workshop Inquiry form goes live on the next staging deploy (#46). Active engineering: deferred-tech follow-ups + Phase 5 polish.
...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | ------------ |
| C-1 | Testimonial re-collection with full attribution (target 12-15). Podcast/channel mining (2026-06-12) found 5 starting points: 3 on-tape leads + 2 published named-client workshop shorts — names and timestamps in the private `../podcast/episodes/CLIP_CANDIDATES.md` §4                                                                                                                                           | 🟡     | Content lead |
...
| C-5 | Client logos — **permission resolved** (the logos already run publicly on the current seqtek.com site, so display rights are established; no fresh permission chase needed). Remaining work is design, not legal: pull the client set off the current site and produce improved, on-brand versions of each mark. The CrossCo workshop client cleared separately for the Touchstone proof section (PR #50)           | 🟢     | Kenn         |
...
- [ ] **Audit & prune dependency overrides** — `package.json#overrides` accrete to clear transitive security advisories (`esbuild` #52, `ws` #66) but nothing flags them once upstream ships a fix, so they rot. Tooling shipped on branch `chore/deps-override-hygiene` (parked): `tools/check-stale-overrides` (an override is stale iff removing it introduces no new high+ prod advisory beyond the baseline) + `.github/workflows/deps-hygiene.yml` (weekly, opens/updates a single `deps-hygiene` tracking issue) + `_overridesNotes` annotations. **Land after #66 merges `ws` to main** (the checker reports a dirty baseline until then), and **backfill the `ws` / `happy-dom` `_overridesNotes` entries** once those overrides are on main. Same shape as the schema-drift CI guard above: an automated check for a hygiene problem that otherwise rots silently.
