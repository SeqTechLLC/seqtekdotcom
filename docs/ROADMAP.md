...
- **NAV-1 — build the dropdown panels, then the pages under them.** _(Reverses the flat-nav decision recorded
  ...
  **Three tiers of content, and only two of them are required.**
  - **Nav button — required.** One page per axis. Hank's narrative lives here. _(Built: both axes are
    ...
    **Done — `/services` collapsed into `/services/what-we-do`.** It had become an orphan: nothing in the
    nav linked it, yet it was the destination of 21 legacy Wix 301s, a sitemap `STATIC_PATHS` entry, the
    "Services" card on the 404 and 410 pages, and the middle breadcrumb crumb on every service route. The axis
    page IS the overview of what we do, so one page does that job instead of an index sitting above two axes.

    All 21 redirects retarget the axis **directly** rather than chaining through `/services` (the no-second-hop
    rule), `/services` itself 301s there for anything bookmarked, `services/page.tsx` is deleted, the
    `service-overview` Page is retired in the content repo, the breadcrumb drops the crumb it can no longer
    justify, and `/services/what-we-do` joins `KNOWN_DETAIL_DESTINATIONS` — which is honest now, on the same
    "listed because the content exists" standard as the workshop entries.
