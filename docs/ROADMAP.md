...
**Spec 008 — GTM pixel activation (split: code/doc shipped; external-config tail + deferrals open):**

- [x] ~~**US3 — conversion-signal surface**~~ — **shipped:** one SSR-safe `pushDataLayer` (`src/lib/analytics/dataLayer.ts`) with `cta_click` + `case_study_view` live (E2E `datalayer-events.e2e.spec.ts`), `booking_complete` as a dormant seam; `form_submission_*` refactored onto the shared emitter (shapes unchanged). Events documented in INTEGRATIONS.md §2.4.
- [x] ~~**US4 — CAPI consent decision**~~ — **recorded** in INTEGRATIONS.md §2.3: the site originates no CAPI events; enforcement (if CAPI continues post-cutover) is at the source, off-site; owners Megan/Domanick.
