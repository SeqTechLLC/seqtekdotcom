...
- [x] T044 [US4] Extended `tests/int/adminMetadata.int.spec.ts` with C4 clauses (1) and (2) — **both amended, because both were unimplementable as written**. Clause (1) as drafted cannot fail: with no `label` declared, the declared label IS the auto-generated one, so they never differ. It now reads on the EFFECTIVE label and fails any mechanically title-cased acronym (`Cta`, `Seo`, `Og Image`, `Url`, `Linkedin`, `Hubspot`), which also catches a hand-typed `label: 'Seo'`. Clause (2)'s "non-obvious list" is not in the spec, and hand-listing 200 paths asserted to hold a non-empty string is the ceremony that got C5 retracted — the set is defined by rule instead (declares `admin.condition`, or is a `select`, or its name carries jargon), and hidden fields are exempt. **T005's flattener was retracted with C5, so it was rebuilt** at `tests/int/helpers/flattenFields.ts`, now also propagating `admin.hidden` to descendants. Both clauses were negative-tested by breaking a label and a description and watching them fail. Contract C4 carries the amendments.
- [x] T045 [US4] `tests/e2e/admin/variantFields.e2e.spec.ts`, on the T002 fixture. **The expectations are computed from the config, not typed out**: each block's own `condition` predicate is evaluated in the test process against the sibling data an option produces, and the DOM is asserted to match — so a conditional field added later is covered the moment it is declared, and a select whose options change nothing is skipped rather than driven. 24 cases across 8 blocks. It asserts the visible set as well as the hidden one, so a locator-convention change fails loudly instead of passing vacuously. The cases are **seeded as one row each and asserted in a single page load**, with one further test that drives the select in the browser: `passesCondition` is computed server-side in `fieldSchemasToFormState` (the client's `WatchCondition` only reads it), so every option change costs a `form-state` round trip and a sequence of them races — a reply built before the last change lands after it and resets the control. Three other things the implementation had to learn: Payload does not render a block row's fields until the row is **in the viewport**, so on a long page everything below the fold reads exactly like a failing condition; a field is located by `#field-<path>`, not by its label, because an `array` heads itself with an `<h3>` and `relationship` and the scalar fields disagree about whether the form uuid is appended to `for`; and the session is minted once and re-attached per test (`AdminSession.attachTo`, new) rather than re-seeded.

### Implementation for User Story 4

- [x] T046 [US4] `seoField()` at `src/payload/fields/seo.ts`, applied to all 10 collections. The ten inline copies were byte-identical to each other, so the extraction is schema-identical by construction. **Evidence**: `npm run generate:types` across the change produced 1021 added / 10 removed lines and **zero changed type lines** — every line of the diff is a JSDoc comment carrying the new help text. Gained a `noun` option ("leave blank to use the case study title") and a `hidden` option, which is what ROADMAP INERT-1 needed.
- [x] T047 [US4] `ctaField()` at `src/payload/fields/cta.ts`, applied to **10** call sites, not 11: `cta` is ×1 (`two-column`), not ×3 — the other two left with Navigation and the withdrawn globals in T016. `primaryCta` ×5, `secondaryCta` ×4. Same zero-type-line-diff evidence as T046. `withStyle` keeps `hero.primaryCta`'s extra `variant` column opt-in, since no other call site has it.
- [x] T048 [US4] Labels written across all 14 collections, the `homepage` global, 45 layout blocks and 7 rich-text blocks: **364 of 369 fields had no explicit label at the start, 5 do now**, and all five sit inside INERT-1 containers an editor cannot open (`locations.address.*`, `industries.clientLogos.logo`) with auto-labels that are already correct English. Two more shapes were extracted rather than repeated — `headingField()` (29 call sites) and `eyebrowField()` (4) at `src/payload/fields/blockCopy.ts`. `headingField` takes the renderer's own fallback line, because ten blocks substitute a hard-coded heading when it is blank and "leave blank for no heading" would be false on those.
- [x] T049 [US4] Help text added wherever C4 clause (2)'s rule bites, and well beyond it where the panel was still opaque: **19 fields carried a description at the start**. The wording is checked against the renderer, not guessed — `metadata.ts`'s fallback chain for the SEO group, `resolveLayout.ts` for which `source` values ignore which picks, `Image.tsx`'s width classes for when alignment stops mattering. Three existing descriptions were rewritten because they were written for a developer ("HubSpot form GUID", "S3 URL to the asset") and one because it was **false**: `related-posts.manualItems` claimed the block "falls back to a category-derived list at render time", and no such resolver exists.
- [x] T050 [US4] Audit recorded per block in `specs/011-payload-admin-ux/variant-audit.md`, all 45. It changed two of them, and **one was a live defect**: `logo-bar.logos` had carried `...requiredWhen(d => d?.source === 'inline')` since spec 003 with `admin: { components: { RowLabel } }` written **after** the spread, so the later key replaced the helper's whole `admin` object, condition included. It validated conditionally and displayed unconditionally — an editor choosing "Reuse the homepage set" still saw an empty logo array asking to be filled in, and filling it in rendered nothing. `requiredWhen` now takes extra `admin` properties as a second argument so the footgun cannot recur. The other change is `image.alignment`, hidden at full width where the three alignment classes are no-ops. Three blocks were examined and deliberately left alone, with reasons, in the audit's notes — `team-grid.filter` in particular, because hiding a required field with no default makes the form unsavable with an invisible error.
- [x] T051 [US4] `src/components/admin/BlockRowLabel.tsx` (beside `MediaRowLabel`, not in `src/payload/admin/` — that directory does not exist and the importMap already resolves `/components/admin`). Wired through `blockAdmin()` rather than "on the blocks where it helps", so **all 45 get it from one edit**. Payload's custom `Label` REPLACES the whole header rather than appending to it, so the number and the type pill are re-created with Payload's own class names — losing them would answer "what is this block about?" at the cost of "what kind of block is it?". `blockAdmin()`'s third argument changed from the preview's alt text to the block's name, which now feeds the alt text, the row pill and (asserted) `labels.singular`; the alt strings were all `'<name> block preview'` already. `generate:importmap` re-run.

### Discovered during implementation (not in the original plan)

- [x] T048a [US4] **ROADMAP INERT-1 landed here**, as the roadmap said it should ("they should be hidden from the admin (`admin.hidden`) so nobody fills in a control that does nothing — a US4 form-legibility task, not a schema change"). 24 fields on the four unrouted collections are now `admin.hidden`: `industries.description/relevantServices/clientLogos`, `locations.description/hasOffice/address.*`, `servicePillars.description/heroImage`, and the `seo` group on all four. The columns stay, because SVC-2 and IND-1 intend to build the routes that read them. `admin.hidden` does not touch REST, so `tools/payload-seed` and the gitignored `docs/content-drafts/*.json` are unaffected.
- [x] T048b [US4] `tests/e2e/admin/blockPicker.e2e.spec.ts` updated: it located the picker button by `/add layout/i`. Payload names that button from the field's `labels.singular`, **not** its `label` — the first attempt at this assumed the label ("Page content") and the button still read "Add Layout". Setting `labels: { singular: 'Block' }` on every `blocks` field is what makes it read "Add Block", which is also better copy; the matcher is now `/add block/i`.
- [x] T050a [US4] `requiredWhen()` gained an optional second argument for the rest of the field's `admin` properties, and the doc comment now says why. See T050.
- [x] T045a [US4] `AdminSession` gained `attachTo(context)` so a spec can mint one session in `beforeAll` and re-attach it per test. `useAdminSession` re-seeds, and re-seeding deletes and recreates the fixture user — which is why the variant suite alternated pass/fail before this.
- [x] T045b [US4] **A local-only trap, documented in `docs/LOCAL_DEVELOPMENT.md`**: a second dev server on any port other than 3100 must be started with `NEXT_PUBLIC_SITE_URL` matching that port. Payload pushes `config.serverURL` onto `config.csrf` and `extractJWT` drops the session cookie when the request's `Origin` is not on that list, so page loads (which send no `Origin`) authenticate while every server action returns `UnauthorizedError`. The admin looks signed in and silently does nothing: no block can be added, no upload preview populates, no conditional field re-evaluates. It cost an hour here, including a wrong "pre-existing on main" conclusion — `main` fails the same way for the same reason.
