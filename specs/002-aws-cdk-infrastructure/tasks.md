...
- [x] T009 Implement `infra/lib/network-stack.ts` — `Vpc` with 2 AZs, public + private + isolated subnets; `NatGateway` count 1; four security groups (`AlbSg`, `AppSg`, `RdsSg`, `LambdaSg`) per `data-model.md` § 1; stack outputs for VPC ID, subnet IDs, SG IDs. Invokes `DeployRoles` so OIDC + per-env deploy role land with the network stack (rare-change-rate blast radius).
