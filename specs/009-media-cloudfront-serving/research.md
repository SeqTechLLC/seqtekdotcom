...
**Rationale**: ARCHITECTURE §6 line 572 justifies the long-TTL `CachingOptimized` policy with "any content change produces a new key — cache busting happens naturally". That was written for `<media-id>/<filename>` keys. With stable `media/<filename>` keys, replacing a file **under the same filename keeps the same key** → the edge serves stale bytes up to a year. Payload dedupes filenames for _new_ docs, so this bites only on replace-in-place — rare, but silent and long-lived when it happens. The helper already exists for page revalidation; the hook is ~15 lines + test. _(Correction found at staging verification: the `CLOUDFRONT_DISTRIBUTION_ID` env var and the `cloudfront:CreateInvalidation` IAM grant had **never** been provisioned — the page-invalidation path had silently skipped since launch (zero invalidations on the staging distribution), masked by 60–120s page TTLs. Fixed in the FR-011 follow-up PR: EdgeStack provisions a `cloudfront_distribution_id` SSM param and attaches the distribution-scoped grant to the instance role.)_ Documenting the gap instead was considered and rejected: Constitution III forces the §6 edit either way, and "known to serve stale for a year" is a poor sentence to write into ARCHITECTURE when the fix is this small.
