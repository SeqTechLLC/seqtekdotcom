...
- `src/lib/payload.ts` (1–284) — the singleton `getPayloadInstance()` plus **16 cached public readers** (3 globals `getSiteSettings`/`getNavigation`/`getHomepage` via `cache(unstable_cache(...))`; 6 detail `getXBySlug`; 6 listing `listX`; `publishedSlugsFor`). The 3 exported **raw helpers** `findPublishedBySlug`/`findPublishedList`/`findPublishedSlugs` run _inside_ the `unstable_cache` callback and are test-only direct entry points — they are **not** wrapped. The timeout wraps the 16 cached readers as a new **outermost** layer; note `publishedSlugsFor`/`listServices`/`listServicePillars` are also reached from the ISR `sitemap.ts` (`revalidate = 3600`), so the wrapper's `headers()` read is guarded for the non-request scope.
