...
- **Phase 3 US1**: framework + MVP block set landed (T044, T049, T064–T070, T072). MVP set is **3 of ~36 layout blocks**: `hero`, `content`, `cta-section` — enough for the SC-001 3-block authoring path. Remaining configs/renderers (T042–T043, T045–T048, T050–T063 partial, T071, T073, including the BLOCK_LIBRARY.md §5.1–§5.6 catalog and the Playwright/coverage tests) are tracked for follow-up PRs. The dispatcher, registry, RichText converter, and Pages.layout wire are in place — adding a new block is 5 mechanical files per BLOCK_LIBRARY.md §9 rule 6.
...
- [ ] T108 [P] [US5] Vitest int `tests/int/collections/access.test.ts` — data-driven matrix iterating 13 collections × 3 roles (public/editor/admin) × 5 ops (read-published, read-draft, create, update, delete) against Payload Local API; asserts each result matches ARCHITECTURE.md §6 (R-17, SC-005)
- [ ] T109 [P] [US5] Vitest int `tests/int/collections/draftLeak.test.ts` — for each draftable collection + global, create a draft and assert unauthenticated REST list, REST detail (404 not 401), and GraphQL queries do NOT return it (SC-006, contract `public-api-draft-filter.md`)
- [ ] T110 [P] [US5] Playwright E2E `tests/e2e/auth/firstSignInBootstrap.spec.ts` — fresh DB; first SSO sign-in becomes admin, second becomes editor (SC-009, FR-036, FR-037)
- [ ] T111 [P] [US5] Playwright E2E `tests/e2e/admin/editorDeleteForbidden.spec.ts` — UI affordance smoke: editor session does NOT see a delete button on `caseStudies` (UI smoke per R-17)

### Implementation / verification

- [ ] T112 [US5] Assert FR-017 regression guard in `tests/int/collections/access.test.ts` (or a sibling test): `users` collection still has Payload's local strategy disabled per spec 001 — `/admin` offers only Google SSO (FR-017, FR-018)
- [ ] T113 [US5] Run T108–T111; reconcile any drift between `docs/ARCHITECTURE.md` §6 and implementation in this PR (Constitution III)
