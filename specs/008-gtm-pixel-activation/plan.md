...
- **II. Tests Gate Merge** — ✅ with a documented carve-out. US3 (`cta_click`, `case_study_view`) ships with E2E `window.dataLayer` assertions. US1 (live container fire-matrix) and US4 (CAPI decision) are **config/decision deliverables verified on staging / in docs**, not CI-testable — identical posture to spec 006's gated tail (T009/T019). The CI-provable substrate (consent gating) is already green from 006. No coverage padding.
