...
- [x] T009 Wire the plugin in `src/payload.config.ts`: register `Accounts` in the collections array, add `authPlugin({ name: 'seqtek', usersCollectionSlug: 'users', accountsCollectionSlug: 'accounts', allowOAuthAutoSignUp: true, successRedirectPath: '/admin', errorRedirectPath: '/admin/login', providers: [GoogleAuthProvider({ client_id, client_secret, params: { hd: 'seqtechllc.com', prompt: 'select_account' } })] })`. Note: endpoint overrides from `contracts/oauth-routes.md` § 2 are dropped — `payload-auth-plugin@0.7.13` does not support them; resolution in the top-of-file FR-012 note.
...
- [x] T014 Run `npm run generate:importMap` and `npm run generate:types`, then commit the regenerated `src/app/(payload)/admin/importMap.js` and `src/payload-types.ts`. (Required after T006 + T010 + T010a add client-component paths and a new collection.)
