...
- [ ] T061 [P] [US4] Integration test proving a new block (`gallery`) renders on multiple page types (page + case study + workshop) with no per-type code, in `tests/int/blocks/blockReuseAcrossTypes.int.spec.ts`
- [ ] T062 [US4] Document the block-curation loop (missing capability → fix/add block → document → available everywhere) in `docs/BLOCK_LIBRARY.md`; update inventory to 45 blocks (FR-011)
...
- [ ] T070 [P] [US6] Fixture: `convert-to-blocks` reproduces a known record's content (or names one gap) and is re-runnable (stable output), in `tests/int/skills/convertToBlocks.int.spec.ts` per contracts/conversion-skill.md
- [ ] T071 [US6] Implement `.claude/skills/convert-to-blocks/SKILL.md` (frontmatter + procedure), distinct from the per-type seed composer and the authoring skill (FR-014)
